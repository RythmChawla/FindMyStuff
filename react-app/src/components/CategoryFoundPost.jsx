...
function CategoryFoundPost(){
    const p = useParams()
    const[posts,setposts]=useState([]);
    useEffect(()=>{
        const data=p.CatId
        const url = 'http://localhost:4000/found/get-category/'+data;
        axios.post(url)
        .then((res)=>{
            if(res.data.posts){
    ...
    return(
        ...
        <div >
            <Header/>
            <Link to='/found'><button>ALL RESULTS</button></Link>
...
export default CategoryFoundPost;